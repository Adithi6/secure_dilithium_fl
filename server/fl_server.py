...
class FederatedServer:
    ...
    def verify_and_aggregate(self, submissions: list[dict], round_num: int) -> list[dict]:
        ...
        for sub in submissions:
            ...
            msg_hash   = sub["msg_hash"]
            signature  = sub["signature"]
            update_bytes = sub["update_bytes"]

            # Step 1: recompute hash from actual received bytes
            recomputed_hash = hashlib.sha256(update_bytes).digest()

            # Step 2: compare with claimed hash
            if recomputed_hash != msg_hash:
                is_valid = False
                verify_ms = 0.0
                status = "REJECTED (HASH MISMATCH)"
            else:
                # Step 3: verify signature only if hash matches
                is_valid, verify_ms = dilithium_utils.verify(pk, msg_hash, signature)
